...
export async function checkScamWithGradio(
  userInput: string
): Promise<{ label: string; explanation: string; confidence: number }> {
  const API_BASE =
    "https://hackerbyhobby-sms-scam-detection.hf.space/gradio_api/call/smishing_detector";
  const payload = {
    data: ["Text", userInput, null],
  };
  ...
  const eventId =
    postJson.event_id || (typeof postJson === "string" ? postJson : null);
  if (!eventId) throw new Error("No event_id returned!");
